...
6. Persist results to local audit targets (MS Access, PostgreSQL, and/or flat files).

If direct Access writes are not available, the utility can also write flat audit files for later import.

When PostgreSQL is enabled, the Node runner writes to:

- `piqi_evaluation_run` (one row per batch run)
- `piqi_evaluation_results` (detailed assessment rows linked by `run_id`)

The `piqi_evaluation_run` row includes:
...
### Verify PostgreSQL schema (verify-first, update only if needed)

Use the checker script to validate required PostgreSQL tables, columns, foreign key, and indexes. It only applies additive updates when required objects are missing.

```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\check-postgres-db.ps1
```

If `psql.exe` is not on your PATH, pass it explicitly:

```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\check-postgres-db.ps1 -PsqlPath "C:\Program Files\PostgreSQL\18\pgAdmin 4\runtime\psql.exe"
```

Optional parameters:

- `-PgHost`, `-Port`, `-Database`, `-User`, `-Password`
- `-PsqlPath` for explicit `psql.exe` location
...
This creates/updates one PostgreSQL evaluation run row for the batch and writes extracted assessment rows with `run_id` and worksheet `row_num`.

Run lifecycle in PostgreSQL:

- Batch start inserts one `piqi_evaluation_run` row with status `IN_PROGRESS`
- `piqi_model_mnemonic` and `evaluation_rubric_mnemonic` are captured from CLI options
- `total_evaluations` and `total_records` are initialized from input row count
- `total_completed` and `total_failed` are updated per processed row
- `completed_at` is set when the run finishes and status transitions to `COMPLETED` or `FAILED`
...
- `process:lab` auto-verifies PostgreSQL run/results tables when PostgreSQL options are provided.
- `process:lab` can run with Access only, flat files only, or both.
- PostgreSQL runtime writes target only `piqi_evaluation_run` and `piqi_evaluation_results`.
