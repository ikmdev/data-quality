...
const { initializeAccessAuditStore, openConnection: openAccessConnection, insertAssessmentResults: insertAccessAssessmentResults, normalizeTableName } = require('../db/access-db');
const {
    createPool,
    ...
    insertAssessmentResults: insertPgAssessmentResults,
    normalizeTableName: normalizePgTableName
...
function printUsage() {
    ...
    console.log('  --table-name <name>                Access audit table or PostgreSQL evaluation run table (default: PiqiAuditLog / piqi_audit_log)');
    console.log('  --assessment-table-name <name>     Assessment results table (default: PiqiAssessmentResults / piqi_evaluation_results)');
...
function buildTimestampRunName() {
    const now = new Date();
    ...
    return `process-lab-${year}${month}${day}-${hours}${minutes}${seconds}-${millis}`;
}
...
async function processRows(options) {
    ...
    const runId = buildTimestampRunName();
    let accessConnection = null;
    let pgPool = null;
    let pgClient = null;
    let pgRunDbId = null;
    let pgRunStatusFinalized = false;
    let flatFileAudit = null;

    // For PostgreSQL, always use the canonical run/results table names
    const pgTableName = 'piqi_evaluation_run';
    const pgAssessmentTableName = 'piqi_evaluation_results';
    ...
    if (usePostgres(options)) {
        ...
        pgRunDbId = await createEvaluationRun(pgClient, pgTableName, {
            runName: runId,
    ...
    try {
        for (let rowOffset = 0; rowOffset < dataRows.length; rowOffset += 1) {
            ...
            if (pgClient) {
                ...
                if (submissionResult.isSuccess && submissionResult.responseBodyText && submissionResult.responseBodyText !== 'DRY_RUN') {
                    ...
                    if (assessmentRecords.length > 0) {
                        const pgAssessmentRecords = assessmentRecords.map((record) => ({
                            ...record,
                            runId: pgRunDbId,
                            rowNum: worksheetRowNumber
                        }));

                        await insertPgAssessmentResults(pgClient, pgAssessmentTableName, pgAssessmentRecords);
                    }
                }

                await incrementEvaluationRunCounts(
                    pgClient,
                    pgTableName,
                    pgRunDbId,
                    submissionResult.isSuccess ? 1 : 0,
                    submissionResult.isSuccess ? 0 : 1
                );
        ...
        if (pgClient && pgRunDbId) {
            await finalizeEvaluationRun(pgClient, pgTableName, pgRunDbId, 'COMPLETED');
            pgRunStatusFinalized = true;
        }
        ...
        if (pgClient && pgRunDbId && !pgRunStatusFinalized) {
            try {
                await finalizeEvaluationRun(pgClient, pgTableName, pgRunDbId, 'FAILED');
                pgRunStatusFinalized = true;
            } catch (finalizeError) {
                console.warn(`Failed to finalize PostgreSQL run status: ${finalizeError.message}`);
            }
        }
        throw error;
    } finally {
